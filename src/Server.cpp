...
namespace Zest {
    ...
    void Session::start() {
        ...
        unsigned short int remote_port =
            std::visit([](auto &s) { return s.lowest_layer().remote_endpoint().port(); }, stream_);
    ...
    void Session::do_read_command(uint32_t payload_size) {
        ...
        std::visit(
            [this, self, payload_size](auto &stream) {
                asio::async_read(
                    stream, this->buffer_, asio::transfer_exactly(payload_size),
                            ...
                            while (offset < payload_size && !should_close) {
                                if (offset + 4 > payload_size) {
                                    final_result += (final_result.empty() ? "" : "\r\n\r\n") +
                                                    ZestDB::responseToJson(
                                                        { ResultType::Code::ERROR, Messages::MALFORMED_BATCH, 0 });
                                    break;
                                }
                                    ...
                                    if (cmd.compare(0, authPrefix.length(), authPrefix) == 0) {
                                        ...
                                        if (dotPos != std::string::npos) {
                                            ...
                                            if (this->db_.validateToken(username, token)) {
                                                ...
                                                cmd_result = ZestDB::responseToJson(
                                                    { ResultType::Code::SUCCESS, Messages::AUTH_SUCCESS, 0 });
                                            } else {
                                                cmd_result = ZestDB::responseToJson(
                                                    { ResultType::Code::ERROR, Messages::AUTH_FAILED, 0 });
                                                should_close = true;
                                            }
                                        }
                                    } else {
